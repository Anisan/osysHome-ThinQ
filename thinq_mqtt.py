...
from plugins.ThinQ.thinq2.controller.thinq import ThinQ
...
LANGUAGE_CODE = "ru-RU" #os.environ.get("LANGUAGE_CODE", "ru-RU")
COUNTRY_CODE = "RU" #os.environ.get("COUNTRY_CODE", "RU")
...
try:
    ...
    print (json.dumps(devices.__dict__))
for device in devices.items:
    ...
    try:
        ...
        for key, value in device.snapshot.state.items():
             print(key,value)
             mqtt_client.publish(mqtt_topic + "/" + device.device_id + "/state/"+key,value, mqtt_qos, True)
    except  Exception as error:
        # Will only catch any other exception
        print("Error: " + str(error))
        #raise
            ...
            for k, v in d.items():
                if isinstance(v, dict):
                    ...
                    try:
                        ...
                        mqtt_client.publish(mqtt_topic + "/" + DeviceID + "/state/" + k,v, mqtt_qos, True)
